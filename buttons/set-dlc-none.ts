...
export default {
    ...
    execute: async (interaction: ButtonInteraction, args: string[]) => {
        ...
        let newDlcs: string[], newDisabledDlcs: string[], newCivs, newDisabledCivs
        newDlcs = []
        newDisabledDlcs = [...game.state.disabledDlcs, ...game.state.dlcs].filter(uniqueFilter);
